...
Smoke прекращается fail-closed при любой неопределённости; blind cleanup mutation после unknown запрещена и требуется ручная проверка test-филиала. Для диагностики summary может вывести только allowlisted `unknown_kind` (`transport`, `http_status`, `response_shape`) и строгий HTTP status `100..599`; exception text, response body, headers, IDs и ПД запрещены. Phase становится live-complete только при сохранённом evidence успешного smoke и локального outcome-unknown E2E. Staging rollback gate закрывается в том же релизе только отдельным distinct app-image циклом `candidate → previous → candidate`, без DB downgrade.
