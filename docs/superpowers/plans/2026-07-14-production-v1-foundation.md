...
The Foundation eval runner must not import future guardrails at module load. Load them lazily for adversarial mode and return a safe `status=unavailable` no-op only while the feature is disabled or the expected module itself is absent. If an existing guardrails module raises any import-time `Exception` or lacks `check_input`, emit safe type-only `status=error` and exit non-zero without traceback, raw exception text or filesystem paths; do not swallow `BaseException`. Preserve empty datasets as an explicit success no-op, and make unreadable/malformed datasets or LLM initialization failures produce a non-zero CLI exit without exposing raw exception text.

## Completion

- [x] Tasks 1–6 implemented and reviewed.
- [x] Independent whole-branch review from merge base approved with no Critical, Important or Minor findings.
- [x] Fresh verification-before-completion gate rebuilt the Docker test image without cache and passed `122/122` tests; isolated containers, volumes, network and test image were removed.
- [x] Telegram polling, merge and push were not performed.
