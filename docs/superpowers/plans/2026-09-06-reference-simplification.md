...
### Правила ветки и коммитов

- Вся эта инициатива продолжается в существующем linked worktree и ветке codex/reference-simplification; отдельная ветка на каждый файл или последовательный шаг не нужна.
- Один законченный и проверенный шаг — отдельный локальный коммит. Измерительные тесты, исправления поведения и последующие content-правки не смешивать без необходимости.
- Перед каждым шагом проверять branch/status; перед объединением заново сверить актуальный main, выполнить итоговый gate на объединяемом кандидате и review. Возможность fast-forward сейчас не гарантирует её после чужих изменений main.
- Не переписывать историю (amend/rebase/reset), не объединять и не отправлять на GitHub автоматически. Push и rollout требуют отдельных решений владельца. Локальный коммит не является удалённой резервной копией.
- Старые ветки/worktrees не мешают этой работе; их cleanup — отдельное действие после проверки уникальных коммитов и несохранённых файлов, не часть упрощения Router.
...
- [x] Для `services`, legacy `service`, `topics`, `action/choice`, date/time/staff и веток соединения ответа заполнить таблицу: клиентский сценарий → наблюдаемый нужный результат → текущий механизм → более простой вариант референса → конкретная потеря/выигрыш → проверка → решение. Карту producer/consumer использовать только для поиска связей и безопасной правки, не как оправдание сохранения.
- [x] Сопоставить «цена + запись», изменение предпочтений, ответ только датой, continue переноса, consultation внутри draft. Измерить число реально выполняемых LLM вызовов и источников состояния на пути; не использовать длину JSON как метрику.
- [x] Для используемого поля также проверить упрощение его потребителя или устранение всей ветки. Например, дата/время оправданы не чтением в conversation.py, а сценарием «запиши завтра после 18:00» без повторного ввода уже названного. Сравнить с более простым последовательным сбором референса: лишние уточнения, потеря ограничений, дополнительные LLM-вызовы, число источников состояния и отказоустойчивость. Не считать текущую реализацию единственным способом обеспечить сценарий.
- [x] Если alias/ветка не обеспечивает нужного сценария или гарантии либо сценарий сохраняется проще, подготовить удаление/замену вместе с потребителем и regression-проверкой. Для изменения Router schema, семантики извлечения или UI сначала оформить отдельную узкую spec/план; наличие согласования не подменяет сравнение альтернатив.
- [x] Оставлять механизм только при указанном сценарии, рассмотренном более простом варианте и конкретной проверяемой причине его отклонения. Допустимый итог — отсутствие дальнейшего упрощения после такого сравнения, но не по одному списку consumers. Не вводить новый orchestration framework ради схожести с референсом.

**Аналитический результат задачи 7 (2026-09-06):** §4.2 исходного аудита содержит сценарии, сравнение альтернатив, границы состояний и measured fake-provider calls. Проверка 193 passed (54.62s), усиленный characterization 7 passed (2.34s), test-коммит d208f8d. Это завершение анализа, не реализации Router. Кодовый пакет [B0: целостность продолжения переноса](../specs/2026-09-06-booking-continuation-integrity-design.md) реализован локально (ad2a4be, 297 passed, review без замечаний); актуальный статус — в дорожной карте. Далее узкий контракт legacy cleanup; замена topics требует отдельного решения вместе с Evaluation. Задачи 8–9 и остальные известные дефекты не закрыты.

## Задача 8 — вход в запись и техническая свежесть

- [x] Проверен backend входа/FAQ/контекстных кнопок: conversational E2E, B0 и два FAQ → та же кнопка. Постоянное меню не возвращается. Это fake-provider проверка, live-классификация и визуальная Telegram-приёмка остаются отдельно.
- [x] Текущий порядок сохранён; для доказанного дефекта четвёртой записи реализованы показанные владельцу короткие страницы B2, без нового меню.
- [x] Sync 1 час и TTL 24 часа подтверждены кодом и тестами: последствия пропуска, ошибки и изменения услуги описаны в §4.4 исходного аудита. Консультация независима от snapshot.
- [x] Частота и TTL сохранены. Изменение требует отдельного SLA; серверное расписание не менялось.

## Задача 9 — релизные условия и завершение общей цели

- [x] Все семь дефектов побочного аудита сверены и локально исправлены: Redis/checksum в пакете A, mixed outage, B0 kind/фазы/исходное время, B2 специалист/четвёртая запись. Проверки и границы — в итоговом отчёте.
- [x] Различия сведены в §4.5 исходного аудита: сценарий, более простой вариант и конкретная потеря. Наличие consumer не использовано как единственное обоснование.
- [x] Подготовлен exact кандидат c5ac62e и [итоговая приёмка](<../../audits/Приёмка упрощения архитектуры 2026-09-06.md>): full 2379 passed/1 document failure, после документной коррекции unit 1315 passed, runtime/tests не менялись. Whole-branch review без существенных замечаний. Это full + post-fix, не зелёный полный повтор; live не проверено.
- [ ] Только после отдельного разрешения выполнить staging rollout с backup prompt/rollback и ручную Telegram-приёмку точного кандидата; merge/push/production отдельно.
