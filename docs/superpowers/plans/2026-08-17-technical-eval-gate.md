...
- [x] Конкретный date+time с датой до/после диапазона или time-only остаётся `invented_slot`, даже если рядом сказано «доступна без записи»; разрешён только чистый ежедневный общий диапазон часов.
