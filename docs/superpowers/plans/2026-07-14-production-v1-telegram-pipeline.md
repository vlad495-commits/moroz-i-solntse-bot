...
- [x] **Step 3:** Through the Docker E2E harness with fake LLM and fake Telegram, send one consented test update twice; expect one inbox row, one LLM call and one outbound send. Live bot E2E remains blocked until a separate Telegram test token exists.
