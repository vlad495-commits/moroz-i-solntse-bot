...
### Task 1: IncomingMessage, inbox/outbox и идемпотентность — выполнено
...
- [x] **Step 1: Write failing duplicate test**
...
- [x] **Step 4: Run migration and test**
...
### Task 2: Privacy gate и Telegram webhook — выполнено
...
- [x] **Step 1: Write failing no-consent test**
...
- [x] **Step 3: Implement webhook gate**
...
### Task 3: Redis buffer и durable enqueue — выполнено
...
- [x] **Step 1: Write failing virtual-time test**
...
- [x] **Step 3: Implement Redis list + deadline**
...
- [x] **Step 4: Run test including concurrent flush**
...
- [x] **Step 1:** Run `docker compose --env-file ../.env --profile test run --rm test pytest tests/unit tests/integration/messaging tests/e2e/test_privacy_gate.py tests/e2e/test_message_delivery.py -q`; expect all pass.
- [x] **Step 2:** Run `docker compose --env-file ../.env build bot`, `docker compose --env-file ../.env run --rm --no-deps --entrypoint python bot -m compileall -q /app` and `docker compose --env-file ../.env run --rm --no-deps --entrypoint python bot -c "import cache, config, db, handlers, llm"`; then run `docker compose --env-file ../.env up -d --build postgres redis rabbitmq admin worker scheduler` and `docker compose --env-file ../.env ps`. Expect smoke success and required non-bot services running. Live Telegram E2E is a staging gate and separately requires a dedicated test-bot token plus HTTPS ingress and `setWebhook` configured with the same secret token; ingress is not added in Phase 2.
- [x] **Step 3:** Through the Docker E2E harness with fake LLM and fake Telegram, send one consented test update twice; expect one inbox row, one LLM call and one outbound send. Live Telegram delivery remains deferred to the staging gate above.
- [x] **Step 4:** Record evidence in roadmap/changelog.
- [x] **Step 5:** Commit with `git commit -m "docs: зафиксирован reliable pipeline checkpoint"`.
