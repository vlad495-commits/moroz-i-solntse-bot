...
Before escalating to support, the official read-only `GET /api/v1/user/permissions/{company_id}` was run with the same Partner+User identity. It confirmed the app is connected to the intended branch and has create/client-add/edit/delete/transfer, but initially lacked client-data/comment/services. The user then enabled those visible developer-app rights and saved; two subsequent permission GETs, including one after a delay, still returned the old false values. The official setup model assigns selected rights to the system user when the app is connected, so the leading root cause is now a stale permission snapshot on the already-connected test integration. Reconnect the same app only in the dedicated test branch, then re-read effective permissions before any mutation. Tokens, company/service/staff configuration and availability remain independently proven.
