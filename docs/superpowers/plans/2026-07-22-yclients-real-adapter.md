...
Before escalating to support, the official read-only `GET /api/v1/user/permissions/{company_id}` was run with the same Partner+User identity. It confirmed the app is connected to the intended branch and has `create_records_access`, `record_form_client_add_access`, `edit_records_access` and `delete_records_access`, but lacks `record_form_client_access`, `records_edit_date_and_master_access`, `records_edit_duration_access`, `records_edit_comment_access` and `records_edit_services_access`. This incomplete effective permission set is the leading configuration root-cause candidate: create sends a synthetic comment, while protected reschedule must preserve client/services/comment and change staff/time/duration. Tokens, company/service/staff configuration and availability are already independently proven. The missing rights must be enabled and re-read through this endpoint before any new mutation decision.
