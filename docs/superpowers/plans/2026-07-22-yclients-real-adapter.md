...
The one explicitly approved post-reconnect smoke again passed availability (`services=1`, `staff=1`, `slots=312`) and returned `mutation_outcome_unknown` at protected create, with no external ID and no later mutation. Now that record/comment visibility is effective, bounded protected GET reconciliation found three active, zero deleted synthetic records with three distinct smoke comment markers. This proves all three prior protected create attempts actually created distinct records; the old zero-count reconciliations were incomplete under the stale visibility rights. The latest response's exact status/shape was intentionally redacted and cannot be recovered post-factum, so no status-mapping fix is justified yet. The phase remains open with three test records requiring an explicit cleanup decision and a future instrumented/TDD-confirmed response-contract investigation; no blind create retry is allowed.
