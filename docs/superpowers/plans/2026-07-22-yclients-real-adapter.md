...
Before escalating to support, the official read-only `GET /api/v1/user/permissions/{company_id}` was run with the same Partner+User identity. It confirmed the app was connected to the intended branch and had create/client-add/edit/delete/transfer, but initially lacked client-data/comment/services. The user enabled those visible developer-app rights and saved; two permission GETs still returned the old false values. After reconnecting the same app only in the dedicated test branch, a fresh isolated Docker verification returned HTTP 200/success and all eight required visible flags true: client-data/client-add/create/edit/comment/services/delete/transfer. This confirms the root cause was the stale system-user permission snapshot and closes the configuration blocker. Tokens, company/service/staff configuration and availability remain independently proven. A new mutation smoke still requires separate explicit consent.

The one explicitly approved post-reconnect smoke again passed availability (`services=1`, `staff=1`, `slots=312`) and returned `mutation_outcome_unknown` at protected create, with no external ID and no later mutation. Now that record/comment visibility is effective, bounded protected GET reconciliation found three active, zero deleted synthetic records with three distinct smoke comment markers. This proves all three prior protected create attempts actually created distinct records; the old zero-count reconciliations were incomplete under the stale visibility rights. With separate explicit user consent, a fail-closed cleanup revalidated exactly those three records by list and individual GET, sent one DELETE per record with no retry, and received three expected successes. Fresh GET-only verification then returned `active=0`, `deleted=3`, `distinct markers=3`. The latest create response's exact status/shape was intentionally redacted and cannot be recovered post-factum, so no status-mapping fix is justified yet. Local TDD now records only safe `transport`/`http_status`/`response_shape` plus a validated HTTP status, never exception text/body/IDs/PII; focused verification is `97 passed`, booking regression is `182 passed`, independent review is `0/0/0`, and the fresh full Docker gate is `436 passed`. A future create still requires explicit consent and must use this instrumented summary; no blind retry is allowed.

The explicitly approved instrumented attempt produced the missing evidence: availability `1/1/312`, then `unknown_kind=response_shape` with HTTP `201`. Protected GET reconciliation found exactly one new active synthetic record. Its ID/comment/services/staff/datetime/duration were consistent, but returned `api_id` was a string different from the submitted owner marker; adapter GET therefore failed closed as not owned. Official create-record documentation describes `api_id` as the external-system identifier, while `comment` is the persisted record comment and `custom_fields` require branch-specific setup. The old ownership design is invalid against the real provider. The new record remains active pending explicit cleanup consent. Before implementation, a replacement recoverable ownership carrier must pass a renewed design approval gate; no mutation or code fix is allowed from inference alone.
