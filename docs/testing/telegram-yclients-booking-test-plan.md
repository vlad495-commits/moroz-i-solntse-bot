...
|---|---|---|
| create → get → reschedule → get → cancel | mock E2E, затем fail-closed sandbox lifecycle и reconciliation | mock готов; sandbox create/get/cleanup доказаны, overlapping-slot defect исправлен и GET-only gate зелёный; полный повтор требует нового отдельного разрешения |
