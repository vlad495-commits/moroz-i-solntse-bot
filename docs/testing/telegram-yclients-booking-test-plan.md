...
| Second new-sandbox lifecycle + slot diagnosis | один отдельно разрешённый `yclients-smoke`, затем только GET reconciliation/permissions/availability diagnosis | 2026-08-04 22:23 | `sandbox` | 1 | `fields/services/staff/slots=2/1/1/322`; create/get confirmed; reschedule definite provider failure; встроенный exact cleanup confirmed; reconciliation `matches=1 active_matches=0`, unknown отсутствует. GET-only measurement подтвердил: service duration `60m`, gap первых стартов `30m`, то есть выбранный reschedule slot перекрывал ещё активную первую запись. Developer UI не предоставляет отдельного `records_edit_date_and_master_access`, поэтому прежняя permission-гипотеза отклонена; повторного lifecycle не было |
| Non-overlapping lifecycle slot regression | TDD выбора безопасной пары в preflight/smoke и affected booking/contract/fail-closed через Docker | 2026-08-04 22:23 | `local-fake` | 0 | Smoke RED `1 failed`; preflight RED `1 failed`; GREEN `2 passed`; final affected `227 passed in 63.67s`, fresh focused gate `80 passed in 0.75s`. Оба runner пропускают overlapping start и требуют второй slot не раньше `first.start + first.duration`; внешний GET-only gate снова `success=true`, `2/1/1/322`, matches `0/0`; independent review `0 Critical / 0 Important / 0 Minor`, `approve`; cleanup `0/0/0/0` |
...
Перед любым mutation профиль fail-closed требует: `YCLIENTS_SANDBOX_CONSENT=I_UNDERSTAND_THIS_CREATES_TEST_BOOKINGS`, `YCLIENTS_ENVIRONMENT_LABEL=sandbox`, имя с префиксом `Synthetic Test `, телефон строго формата `+7000` и ещё семь цифр, а также `YCLIENTS_TEST_WINDOW_DAYS` как целое от `1` до `14`. До каталога он GET-only проверяет оба ownership field. Слоты читаются только в заданном окне; нужны два будущих неперекрывающихся старта, где второй начинается не раньше окончания первого. Затем bounded GET reconciliation по свежему UUID обязан вернуть строго `matches=0`, `active_matches=0`; auth/transport/shape failure или неожиданное совпадение также останавливают flow до mutation.
...
| Second new sandbox lifecycle | один новый отдельно разрешённый runner command с встроенным bounded cleanup | 2026-08-04 22:23 | `sandbox` | 1 | create/get confirmed; reschedule definite provider failure; runner выполнил один exact cleanup cancel, reconciliation `matches=1 active_matches=0`; `manual_review_required=false`, unknown отсутствует. GET-only availability доказал overlapping `30m` gap при duration `60m`; selection fix и зелёный GET-only gate готовы, следующий mutation требует нового отдельного разрешения |
| Final corrected sandbox lifecycle | один новый явно разрешённый `yclients-smoke` после исправления overlapping slot selection | 2026-08-04 22:44 | `sandbox` | 1 | Полная цепочка подтверждена: create/get/reschedule/get/cancel — `confirmed`; `final_state=cancelled`, reconciliation `matches=1 active_matches=0`, `success=true`, `manual_review_required=false`, unknown/error отсутствуют. Использованы только фейковые данные; lifecycle namespace и следующий regression namespace очищены до `0/0/0/0`. Booking/reminders Docker regression завершился с exit `0` |
...
|---|---|---|
| create → get → reschedule → get → cancel | mock E2E, затем fail-closed sandbox lifecycle и reconciliation | готово: mock E2E и исправленный внешний sandbox lifecycle подтверждают всю цепочку; финал `cancelled`, reconciliation `matches=1 active_matches=0`, unknown отсутствует |
...
Runtime gates 2026-08-04 16:32 UTC+3: Compose build `worker scheduler` — exit `0/0`; `compileall /app` в обоих образах — exit `0/0`; Alembic — `0010_telegram_booking_flow (head)`. Успешный external sandbox lifecycle получен 2026-08-04; scheduler в staging по-прежнему намеренно не включён без отдельного разрешения на изменение staging.
