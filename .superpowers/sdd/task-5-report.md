...
- Activation and `set_mode("active")` lock settings/version rows, then acquire fixed-order `SHARE` locks on all five population tables before rechecking `fresh_preview`, `same_checksum`, `current_watermarks`, `test_sent` and `legal_approved`. A coordinated concurrent-writer regression proves the writer waits until activation commits, giving a serial order. The `30:00` boundary is expired.
...
- Final Task 5 + existing outbox suite after review fixes: `67 passed in 158.34s`.
- Affected admin/RBAC/audit regressions after review fixes: `28 passed in 6.34s`.
