...
MIGRATION = Path("/workspace/migrations/versions/0016_llm_validator_evaluations.py")
...
def test_migration_is_additive_and_uses_common_eval_tables():
    ...
    assert 'down_revision = "0015_llm_input_security"' in text
    assert module.revision == "0016_llm_validator"
    assert len(module.revision) <= 32
    assert '"suite": "validator"' in text
    assert "validator_eval_cases" not in text
...
def test_migration_seed_matches_versioned_dataset():
    ...
    assert module.VALIDATOR_CASES == json.loads(DATASET.read_text(encoding="utf-8"))
...
def test_migration_pins_dataset_bytes_and_rejects_tampering(tmp_path):
    ...
    assert module.VALIDATOR_DATASET_SHA256 == hashlib.sha256(canonical).hexdigest()
    tampered = tmp_path / "validator_dataset.json"
    tampered.write_bytes(DATASET.read_bytes() + b" ")
    with pytest.raises(RuntimeError, match="Validator dataset integrity mismatch"):
        module._load_validator_cases(tampered)


def test_migration_dataset_hash_accepts_git_line_ending_conversion(tmp_path):
    module = _load_migration()
    canonical = DATASET.read_bytes().replace(b"\r\n", b"\n")
    crlf_dataset = tmp_path / "validator_dataset.json"
    crlf_dataset.write_bytes(canonical.replace(b"\n", b"\r\n"))

    assert module._load_validator_cases(crlf_dataset) == json.loads(canonical)


def test_downgrade_targets_only_validator_owned_rows():
    text = MIGRATION.read_text(encoding="utf-8")
    downgrade = text.split("def downgrade", 1)[1]

    assert "suite = 'validator'" in downgrade
...
def test_migrate_image_copies_versioned_validator_dataset():
    text = MIGRATE_DOCKERFILE.read_text(encoding="utf-8")

    assert (
        "COPY llm/eval/validator_dataset.json "
        "/app/llm/eval/validator_dataset.json"
    ) in text
