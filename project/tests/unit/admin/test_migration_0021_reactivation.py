...
MIGRATION = Path("/workspace/migrations/versions/0021_admin_reactivation.py")
...
def test_reactivation_migration_owns_only_its_additive_tables():
    ...
    assert 'revision = "0021_admin_reactivation"' in text
    assert 'down_revision = "0020_message_llm_analytics"' in text
    for table in (
        "marketing_consents",
