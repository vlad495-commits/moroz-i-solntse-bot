...
MIGRATION = Path("/workspace/migrations/versions/0022_admin_statistics.py")


def test_statistics_migration_is_additive_and_singleton_scoped():
    text = MIGRATION.read_text(encoding="utf-8")

    assert 'revision = "0022_admin_statistics"' in text
    assert 'down_revision = "0021_admin_reactivation"' in text
