...
def test_migration_image_is_minimal_and_non_root():
    ...
    assert (
        "COPY llm/eval/router_dataset_v3.json "
        "/app/llm/eval/router_dataset_v3.json"
        in dockerfile
    )
...
def test_compose_process_environment_overrides_external_test_credentials():
    ...
    assert services["worker"]["environment"] == {
        ...
        "SECURITY_MAX_TOKENS": "${SECURITY_MAX_TOKENS:-10}",
        "OUTPUT_VALIDATOR_ENABLED": "${OUTPUT_VALIDATOR_ENABLED:-false}",
        "CONTEXT_MESSAGES_LIMIT": "${CONTEXT_MESSAGES_LIMIT:-40}",
        "COMPACT_MAX_TOKENS": "${COMPACT_MAX_TOKENS:-400}",
