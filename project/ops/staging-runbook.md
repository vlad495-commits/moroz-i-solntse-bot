...
Остановиться с identity, DNS или port blocker при любой неоднозначности. Не останавливать и не перенастраивать найденные production/shared процессы.
...
`staging-webhook` проверяет dedicated bot identity до Telegram Bot API операций `getWebhookInfo`, `setWebhook` и `deleteWebhook`. Стандартная установка не очищает pending updates.
...
Identity mismatch, `ok:false`, unexpected pending count или Telegram error — blocker.
