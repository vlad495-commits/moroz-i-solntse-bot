...
async def validator_eval_index(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        "eval_list.html",
        {
            "user": user,
            "suite": "validator",
            "cases": await evdb.list_cases("validator"),
            "problem_cases": await evdb.list_problem_cases("validator"),
            "runs": await evdb.list_runs(10, "validator"),
        },
    )
...
async def validator_eval_run_start(
    request: Request,
    csrf_token: str = Form(""),
):
    user = await get_current_user(request)
    ...
    if not cases:
        return RedirectResponse(
            url=admin_url(request, "/eval/validator/?error=no_cases"),
            status_code=302,
        )

    run_id = await evdb.create_run(
        len(cases),
        eval_runner.VALIDATOR_MODEL,
        "validator",
    )
    _start_eval_task(
        run_id,
        eval_runner.run_validator_eval_set(run_id, cases=cases),
    )
    await record_audit(
        actor_id=user.id,
        action="eval.validator_run_start",
        object_type="eval_run",
        object_id=str(run_id),
        before=None,
...
async def validator_eval_problem_run_start(
    request: Request,
    csrf_token: str = Form(""),
):
    user = await get_current_user(request)
    require_role(user, {"owner"})
    validate_csrf(user, csrf_token)
    cases = await evdb.list_problem_cases("validator")
    if not cases:
        return RedirectResponse(
            url=admin_url(
                request,
                "/eval/validator/?error=no_problem_cases",
            ),
            status_code=302,
        )
    ...
    await record_audit(
        actor_id=user.id,
        action="eval.validator_problem_run_start",
        object_type="eval_run",
        object_id=str(run_id),
        before=None,
        after={"total": len(cases), "suite": "validator"},
