...
- [ ] **BLOCKED (локальная среда):** полный Docker `pytest`-набор на точном интеграционном HEAD. Повторная read-only проверка 2026-08-05 18:40 (UTC+3) сразу вернула отсутствие named pipe `dockerDesktopLinuxEngine`; Docker не перезапускался и не исправлялся.
- [ ] **BLOCKED (локальная среда):** объединённые deterministic scheduler/reminder/failure проверки, simulated alert delivery/dedup/channel-failure и mock eval-harness на точном HEAD. Исторические и focused доказательства есть, но свежий Docker-run текущего этапа не выполнялся.
- [ ] **BLOCKED (локальная среда):** реальный локальный backup → restore в отдельную пустую PostgreSQL → сверка Alembic revision, контрольных count и тестовых сущностей. Production-база не должна использоваться.
- [ ] **BLOCKED (локальная среда):** изолированные Compose build, migrations и healthchecks с синтетическим test `.env`. Внешний release `.env` для этой локальной проверки не требуется и не должен копироваться.
- [ ] **BLOCKED (локальная среда):** полный Ruff/static/security gate внутри test image на точном HEAD. Текущий не-Docker `diff --check` не подменяет этот пункт.

Ограничение среды: test `.env` в текущей worktree отсутствует, но главным блокером стал недоступный Docker server API. Контейнеры и volumes — свои или чужие — в этом этапе не запускались, не останавливались и не удалялись.
