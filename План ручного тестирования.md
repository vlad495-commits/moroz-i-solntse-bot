...
- [ ] **PARTIAL:** compileall, `sh -n`, base/staging/production Compose render и focused security gate `319 passed` завершены, но pinned Ruff `0.12.7` (`--no-cache --select E9,F .`) нашёл `10` F401 в project-коде/тестах и local skill templates. Полный static gate остаётся открытым до отдельного code-fix и свежего повтора.
