...
- [ ] **BLOCKED (локальная среда):** полный Docker `pytest`-набор на точном HEAD. Единственная read-only проверка `docker version --format '{{json .Server}}'` завершилась timeout примерно через 34 секунды; Docker не перезапускался и не исправлялся.
